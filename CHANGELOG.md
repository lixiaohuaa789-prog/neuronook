...
- 若已切到大容量本地存储，会显示 `IndexedDB（大容量完整模式）` 与近似占用，便于确认导入后的实际保存方式。

# v1.0.5 (2026-04-11)

- iPad/低配额浏览器导入修复：当 `localStorage` 空间不足时，完整笔记数据自动切换到 `IndexedDB` 保存，不再移除内嵌图片。
- 导入/恢复提示更新：若启用大容量本地存储，会明确提示“已完整保存到大容量本地存储”。
- 复习子页刷新修复：`mastered/progress/tomorrow` 页面接入统一数据变更事件，导入后可自动看到 IndexedDB 中的数据。

# v1.0.4 (2026-04-11)

- 本地导入容量热修：恢复备份时若触发 Safari/localStorage 配额错误，将自动移除内嵌 base64 图片后重试保存。
- 导入/恢复提示增强：成功后会展示已移除的内嵌图片数量，便于确认为何可导入成功。
